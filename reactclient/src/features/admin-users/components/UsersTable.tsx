...
function statusBadgeType(status: AccountStatus) {
...
interface UsersTableProps {
  ...
  onStatusChange: (user: UserListItemDto, status: AccountStatus) => void;
}

export default function UsersTable({ users, loading, onEdit, onStatusChange }: UsersTableProps) {
  return (
          ...
          {loading ? (
            ...
            users.map((user) => (
              <tr key={user.id} className="hover:bg-surface-container-low/50">
                ...
                <td className="px-lg py-md text-on-surface-variant text-sm">{user.displayName}</td>
                ...
                <td className="px-lg py-md text-right space-x-sm">
                  ...
                  <select
                    ...
                    onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                      onStatusChange(user, e.target.value as AccountStatus)
                    }
