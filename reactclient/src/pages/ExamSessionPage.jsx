...
import { finishSession, abandonSession } from '../services/examService';
import { getSessionErrorMessage, countAnswered, isOptionSelected } from '../utils/sessionAnswers';
...
export default function ExamSessionPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitError(null);
    try {
      const sessionResponses = Object.entries(answers).flatMap(([idx, value]) => {
        const questionId = questions[Number(idx)].questionId;
        const ids = Array.isArray(value) ? value : [value];
        return ids.map((selectedOptionId) => ({ questionId, selectedOptionId }));
      });
