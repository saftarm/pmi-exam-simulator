...
export default function StatusBadge({ status, type = 'neutral' }: StatusBadgeProps) {
  return (
    <span
      className={`px-sm py-xs rounded-full text-xs font-bold ${STATUS_STYLES[type] || STATUS_STYLES.neutral}`}
    >
      {status}
    </span>
  );
  ...
  return (
    <div className="bg-white p-lg rounded-xl border border-outline-variant shadow-sm">
      <div className="flex justify-between items-start mb-md">
        ...
        {change && !loading && <span className={`text-xs font-bold ${changeClass}`}>{change}</span>}
