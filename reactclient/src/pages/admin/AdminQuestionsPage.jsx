...
import { FormSkeleton, Spinner } from '../../components/loading';
        ...
        <AdminLayout title="Question Pool">
            <div className="flex flex-col lg:flex-row justify-between gap-md mb-lg">
                <p className="text-on-surface-variant text-sm max-w-xl">
                    Global question bank shared across exams. Questions are sampled by domain when a learner starts a session.
                </p>
                <div className="flex flex-wrap gap-md">
                    ...
                    <label className={`flex items-center gap-sm px-md py-sm rounded-lg border border-outline-variant font-bold text-sm cursor-pointer hover:bg-surface-container-low transition-opacity ${importing ? 'opacity-70' : ''}`}>
                        {importing ? <Spinner size="sm" label="Importing" /> : <Icon name="upload_file" style={{ fontSize: 18 }} />}
                ...
                <div className="mb-lg p-md bg-red-50 text-red-700 rounded-lg border border-red-200 text-sm">
                    ...
                    {loadError.includes('405') || loadError.toLowerCase().includes('method') ? (
                        <p className="mt-sm text-xs">
                            The API may be running an old build. Stop TestAPI and run <code>dotnet run</code> again.
                        </p>
                    ) : null}
                </div>
            )}
